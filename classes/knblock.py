...
__all__ = ['KnBlock', 'KnBlockException', 'KnBlockParamsException']


class KnBlockException(Exception):
...
class KnBlockParamsException(Exception):
    def __init__(self, value):
        self.value = value
...
class KnBlock:
    def __init__(self, param):
        if param is None:
            raise KnBlockException('param must be specified.')
        else:
    ...
    def get_img(self):
        ...
        if os.path.exists(self.imgfname):
            ...
            if self.img is None:
                raise KnBlockException(self.imgfname + 'cannot be read')
            else:
            ...
            raise KnBlockException('%s not found' % self.imgfname)
    ...
    def collect_boxes(self):
        ...
        """
        self.logger.debug('enterd into KnBlock#collect_boxes')
        if len(self.boxes) == 0:
            self.getBoxesAndCentroids()
    ...
    def collect_boxes_with_debug(self, outdir=None):
        ...
        bounding boxを包含するboxに統合し、文字を囲むboxの取得を試みる
        """
        self.logger.debug('enterd into KnBlock#collect_boxes_with_debug')
        if len(self.boxes) == 0:
            self.getBoxesAndCentroids()
